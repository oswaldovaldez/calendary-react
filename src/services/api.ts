...
// api.ts
const API_URL = import.meta.env.API_URL || "http://localhost:9999/api";
...
export const Api = {
  ...
  logout: (data: { user_id: number }) => apiFetch('/auth/logout', { method: 'POST', body: data }),
  //crud usuarios
  ...
  readUsers: (data: { _token: string }) => apiFetch(`/users`, { method: 'GET', headers: {Authorization:` Bearer ${data._token}`} }),
  updateUser:(data:{name:string,email:string,password:string,role:string,commerce_id:number|null,user_id:number,_token:number})=>apiFetch(`/users/${data.user_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteUser:(data:{user_id:number,_token:number})=>apiFetch(`/users/${data.user_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  updateCommerce:(data:{name:string,email:string,phone:string,data:object,commerce:number,_token:number})=>apiFetch(`/commerces/${data.commerce}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteCommerce:(data:{name:string,email:string,password:string,role:string,commerce_id:number|null,commerce:number,_token:number})=>apiFetch(`/commerces/${data.commerce}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readCategories:(data:{_token:string})=>apiFetch(`/categories`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateCategory:(data:{name:string,description:string,parent_id:number|null,commerce_id:number,category_id:number,_token:number})=>apiFetch(`/categories/${data.category_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteCategory:(data:{category_id:number,_token:number})=>apiFetch(`/categories/${data.category_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readServices:(data:{_token:string})=>apiFetch(`/services`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateService:(data:{name:string,description:string,commerce_id:number,duration:number,price:number,price_offer:number,service_id:number,_token:number})=>apiFetch(`/services/${data.service_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteService:(data:{service_id:number,_token:number})=>apiFetch(`/services/${data.service_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readProducts:(data:{_token:string})=>apiFetch(`/products`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateProduct:(data:{name:string,description:string,commerce_id:number,product_id:number,_token:number})=>apiFetch(`/products/${data.product_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteProduct:(data:{product_id:number,_token:number})=>apiFetch(`/products/${data.product_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readPatients:(data:{_token:string})=>apiFetch(`/patients`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updatePatient:(data:{first_name:string,last_name:string,email:string|null,phone:string|null,birth_date:string|null,gender:string|null,patient_id:number,_token:number})=>apiFetch(`/patients/${data.patient_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deletePatient:(data:{patient_id:number,_token:number})=>apiFetch(`/patients/${data.patient_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readRecordTemplates:(data:{_token:string})=>apiFetch(`/record-templates`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateRecordTemplate:(data:{name:string,fields:object,recordTemplate_id:number,_token:number})=>apiFetch(`/record-templates/${data.recordTemplate_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteRecordTemplate:(data:{recordTemplate_id:number,_token:number})=>apiFetch(`/record-templates/${data.recordTemplate_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readRecords:(data:{_token:string})=>apiFetch(`/records`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateRecord:(data:{patient_id:number,commerce_id:number,record_template_id:number|null,type:string|null,data:object|null,record_id:number,_token:number})=>apiFetch(`/records/${data.record_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteRecord:(data:{record_id:number,_token:number})=>apiFetch(`/records/${data.record_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readSchedules:(data:{_token:string})=>apiFetch(`/schedules`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateSchedule:(data:{day_of_week:string,start_time:string,end_time:string,breaks:object|null,schedule_id:number,_token:number})=>apiFetch(`/schedules/${data.schedule_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteSchedule:(data:{schedule_id:number,_token:number})=>apiFetch(`/schedules/${data.schedule_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  ...
  readAppointments:(data:{_token:string})=>apiFetch(`/appointments`,{method:'GET',headers: {Authorization:` Bearer ${data._token}`}}),
  updateAppointment:(data:{start_at:string,end_at:string,status:string|null,commerce_id:number|null,patient_id:number,schedule_id:number|null,user_id:number,notes:string|null,name:string|null,description:string|null,appointment_id:number,_token:number})=>apiFetch(`/appointments/${data.appointment_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
  deleteAppointment:(data:{appointment_id:number,_token:number})=>apiFetch(`/appointments/${data.appointment_id}`,{method:'PATCH',body:data,headers: {Authorization:` Bearer ${data._token}`}}),
