...
const ShowCategory = () => {
  ...
  if (error || !category) {
    return <div className="card neumo p-6">{error ?? "Categoría no encontrada"}</div>;
  }
  ...
  return (
    <div className="card neumo">
      ...
      <div className="card-body grid gap-3 md:grid-cols-2">
        ...
        <div>
          <span className="font-semibold">Comercio:</span> {category.commerce_id}
        </div>
        <div>
          <span className="font-semibold">Categoría padre:</span> {category.parent_id ?? "Sin asignar"}
        </div>
        <div>
          <span className="font-semibold">Estado:</span> {category.status ? "Activa" : "Inactiva"}
        </div>
        <div>
          <span className="font-semibold">Slug:</span> {category.slug ?? "Sin slug"}
        </div>
        <div>
          <span className="font-semibold">Creada:</span> {category.created_at ? String(category.created_at) : "-"}
        </div>
        <div>
          <span className="font-semibold">Actualizada:</span> {category.updated_at ? String(category.updated_at) : "-"}
        </div>
      </div>
    </div>
  );
